 ...
 */
import momoQR from "/public/momo.jpg";
const TAB_CONTENT = {
  education: {
    icon: "🎓",
  ...
  projects: {
    ...
    desc: "Lười làm chó gì có dự án.",
  },
  hobbies: {
    icon: "❤️",
    title: "Sở thích & Niềm đam mê",
    desc: "Những điều mình yêu thích: gaming, âm nhạc và cậu.",
  },
  contact: {
    icon: "📬",
...
export default function PlaceholderView({ tabId, onBack }) {
  ...
  return (
    <div className="relative z-[1] min-h-screen flex items-center justify-center px-6 py-20">
      <div
        className="animate-fade-up max-w-md w-full text-center p-12 rounded-3xl border backdrop-blur-xl"
        style={{ background: "var(--surface)", borderColor: "var(--border)" }}
      >
        <span className="text-6xl block mb-6" role="img" aria-hidden="true">
          {content.icon}
        </span>
        <h2
        ...
        {tabId === "projects" && (
          <div className="mb-8">
            <img
              src={momoQR}
              alt="MoMo QR - Góp Quỹ nuôi Sáng"
              className="mx-auto rounded-2xl"
              style={{ width: 220, boxShadow: "0 4px 24px rgba(0,0,0,0.25)" }}
            />
            <p className="text-xs mt-3" style={{ color: "var(--text-muted)" }}>
              Quét mã để góp Quỹ nuôi Sáng 🐾
            </p>
          </div>
        )}
        <button
